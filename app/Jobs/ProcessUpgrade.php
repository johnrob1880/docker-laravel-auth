...
class ProcessUpgrade implements ShouldQueue
{
    ...
    public function handle(OmegaWebApiContract $api, ActivityLoggerContract $activityLogger)
    {
        ...
        $link = [
            'type' => 'TestUpgrade',
            'value' => $this->upgrade->testId,
            'description' => $this->upgrade->test
        ];

        $testUpgrade =  $api->linkBarcode($this->barcode, $link);


        if (is_null($testUpgrade))
        {
            // error?? manually fail job??
            
            $activityLogger->create(
                Activities::TEST_UPGRADE_LINK_FAILURE, 
                "Attempt to link test upgrade failed for barcode: " . $this->barcode . '. Link failed.',
        ...
        $activityLogger->create(
            Activities::TEST_UPGRADE_LINK_SUCCESS, 
            "Test upgraded to " . $this->upgrade->test . " successfully linked for barcode: " . $this->barcode,
